...
class FindConcat {
    static class TrieNode {
        ...
        public static TrieNode buildTree(Iterable<String> words, boolean withWord) {
            TrieNode root = new TrieNode();

            for (String w : words) {
                if (w.isEmpty()) continue;
                addWord(root, w, withWord);
            }
        ...
        private static void addWord(TrieNode root, String w, boolean withWord) {
    ...
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
        root = TrieNode.buildTree(Arrays.asList(words), false);
...
//        root.print();
        ...
        for (String w : Arrays.asList("q")) {
            char[] chs = w.toCharArray();
            if (isConcat(chs, 0, chs.length)) {
                res.add(w);
            }
        }
    ...
    private boolean isConcat(char[] chs, int i, int j) {
        System.out.println("checking " + new String(chs, i, j - i));
        TrieNode node = root;
        while (i < j) {
            int index = chs[i] - 'a';
            node = node.children[index];
            if (node == null) {
                System.out.println("no more " + chs[i] + " at index " + index);
                return false;
            }

            if (node.isWord) {
                System.out.println(chs[i] + " at index " + index + " is word");
                if (isWord(chs, i+1, j)) return true;
                if (isConcat(chs, i + 1, j)) {
                    return true;
                }
            } else {
                System.out.println(chs[i] + " at index " + index + " is not word");
            }
            i++;
        }
        return false;
    ...
    private boolean isWord(char[] chs, int i, int j) {
        TrieNode node = root;
        while (i < j) {
            int index = chs[i] - 'a';
            node = node.children[index];
    ...
    public static void main(String[] args) {
        System.out.println(new FindConcat().findAllConcatenatedWordsInADict(new String[]{
